...
const Presets = {
  ...
  /**
   * 获取所有模板（含模型类型与名称），供下拉展示
   * @returns {Array} [{ modelType, name, params }]
   */
  getAllTemplates() {
    const all = [];
    for (const modelType in this.templates) {
      for (const name in this.templates[modelType]) {
        all.push({
          modelType,
          name,
          params: Helpers.deepClone(this.templates[modelType][name]),
        });
      }
    }
    return all;
