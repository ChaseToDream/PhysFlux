...
class ObjectLibraryManager {
  ...
  _bindEvents() {
    ...
    // 保存并应用
    document.getElementById('objSaveCustom').addEventListener('click', () => {
      this._saveCustomObject();
    });
  }
  ...
  _renderObjectGrid() {
    ...
    if (items.length === 0) {
      grid.innerHTML = '<div class="empty-hint">该分类暂无物品</div>';
      return;
    }
  ...
  _renderCustomList() {
    ...
    if (names.length === 0) {
      list.innerHTML = '<div class="empty-hint">暂无自定义物品</div>';
      return;
    }
  ...
  _collectFormData() {
    ...
    if (isNaN(mass) || mass <= 0 || mass > 100) {
      alert('质量必须在 0 ~ 100 kg 之间');
      return null;
    }
    if (isNaN(radius) || radius < 2 || radius > 30) {
      alert('半径必须在 2 ~ 30 px 之间');
      return null;
    }
  ...
  _updateActiveInfo() {
    ...
    if (this.activeObjectName) {
      info.hidden = false;
      nameEl.textContent = this.activeObjectName;
    } else {
      info.hidden = true;
    }
  }
...
window.ObjectLibraryManager = ObjectLibraryManager;
